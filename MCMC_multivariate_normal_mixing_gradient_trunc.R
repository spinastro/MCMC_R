...
y_orig <- matrix(c(y_c[,1],y_f[,1],y_c[,2],y_f[,2]), nrow=N, ncol=2, byrow=F)

L = -5
U=27

hist(y_orig[,1], breaks = 100)
hist(y_orig[,2], breaks = 100)

y = y_orig[(y_orig[,1] > L) & (y_orig[,1] < U),,drop=FALSE]

hist(y[,1], breaks = 100)
hist(y[,2], breaks = 100)

r = dim(y)[1]/dim(y_orig)[1]
f_f_orig = 1-f
f_f = (r*f_f_orig)/(f_f_orig*(r-1)+1)
f_c=1-f_f
################################################# likelihood
likelihood = function(param){
  ...
  if ((f > 0.001) & (f < 1)) {
    singlelikelihoods = f*(dmvnorm(y, c(mu1_c,mu2_c), cov_c)/pmvnorm(lower=c(L,-Inf), upper=c(U,Inf), mean= c(mu1_c,mu2_c), sigma=cov_c)) + (1.-f)*(dmvnorm(y, c(mu1_f,mu2_f), cov_f)/pmvnorm(lower=c(L,-Inf), upper=c(U,Inf), mean=c(mu1_f,mu2_f), sigma=cov_f))
  } else {
    singlelikelihoods = 0
  }
...
################################################# proposal function
proposalfunction = function(param,acc,prev_sig){
  step = rnorm(11, mean = 0, sd= c(0.1,0.1,0.1,0.1,0.05,0.1,0.1,0.1,0.1,0.05,0.01))
  if (acc == 1) {
    step = step*prev_sig/sign(step)
  }
  new_par = param + step
...
run_metropolis_MCMC = function(startvalue, iterations){
  ...
  for (i in 1:iterations){
    ...
    #Sys.sleep(0.01)
    out_proposal = proposalfunction(chain[i,],acc,prev_sig)
    proposal = out_proposal[1:11]
    prev_sig = out_proposal[12:22]
...
posterior_sample = function(n,pars) {
  ...
  y <- matrix(c(y_c[,1],y_f[,1],y_c[,2],y_f[,2]), nrow=dim(y_c)[1]+dim(y_f)[1], ncol=2, byrow=F)
  return(y)
}
...
data = hist(y_orig[,1], nbin, plot=F)
plot(data$mids,data$density, type="l", col="red")

for (i in 1:200){
  f_f_t = (1 - post_pars[burnIn-i,11])
  f_f = f_f_t/(f_f_t+r-r*f_f_t)
  f_c = 1-f_f
  post_pars[burnIn-i,11] = f_c
  ps = posterior_sample(N, post_pars[burnIn-i,])
  data_post = hist(ps[,1], nbin, plot=F)
  lines(data_post$mids,data_post$density, col='#0000FF33', , lwd=0.1)
}

lines(data$mids,data$density, col="red", lwd=5)
