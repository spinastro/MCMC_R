...
################################################# proposal function
proposalfunction_adapt = function(param,sign_adapt,prev_sig,step_scale,S){
  step = rnorm(19, mean = 0, sd= step_scale)
  if (sign_adapt == 100) {
    new_step = step[step_scale != 0]*prev_sig[step_scale != 0]/sign(step[step_scale != 0])
    step[10:18] = new_step[10:18]
  }
  new_par = param + S%*%step
  
    return(list(new_par=new_par, step=step, sign=sign(step)))
}

################################################# run_metropolis_MCMC
run_metropolis_MCMC_robust = function(iterations=1000,startvalues=rep(0,19),prior_scales=rep(1,19), step_scale=c(rep(0.05,6),rep(0.01,3),rep(0.05,6),rep(0.01,3),0.01), low_lim=c(-Inf,-Inf,-Inf), up_lim=c(Inf,Inf,Inf), S=diag(1,19), burnIn=500, obs, obs_err, adapt=FALSE){
  print("#######################")
  ...
  for (i in 1:iterations){
    ...
    out_proposal = proposalfunction_adapt(chain[i,],sign_adapt,prev_sig,step_scale,S)
    proposal = out_proposal$new_par
    prev_sig = out_proposal$sign
    step_values[i+1,] = S%*%step_scale
    #print(proposal)
    #print(S%*%step_scale)
    ...
    if (runif(1) < acceptance_probab[i]) {
      chain[i+1,] = proposal
      N_acc = N_acc + 1
      sign_adapt = 1 # forces to keep the sign of the next step
    }else{
      chain[i+1,] = chain[i,]
      sign_adapt = 0 # free to change the sign of the next step
    }
    if (adapt & i < 5000) {
        S <- adapt_S(S, proposal-chain[i,], min(1,acceptance_probab[i]), i, target=0.234, gamma=0.1)
    }
    acceptance_rate[i] = N_acc/i
    print(i)
    print(c(acceptance_probab[i],acceptance_rate[i]))
    print(chain[i+1,])
  }
